                ...
                if [ $? -eq 0 ]; then
                        ...
                        if [ $? -eq 0 ]; then
                                ...
                                if [ $? -eq 0 ];then
                                        ...
                                        if [ $? -eq 0 ]; then
                                                py_latest=$(python3.9 --version)
                                                echo "Latest Installed Python version is $py_latest"
                                                py_latest_install="python"$(echo $py_latest | cut -d " " -f2)
...
find_requirements_file=$(find . -type f -name "requirements.txt")
...
if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
                ...
                echo "Problem in installing dependencies. Aborting the process..."
        fi
else
        echo "No requirements.txt in the current directory. Please upload it."
fi
