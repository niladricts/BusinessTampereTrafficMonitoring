...
import BusinessTampereTrafficMonitoring.tools.geometry as geom


def test_simple_triangle():
    hull = geom.convex_hull([(0, 0), (4, 4), (8, 0)])
    assert geom.point_inside((3, 2), hull) is True
    assert geom.point_inside((1, 2), hull) is False
    assert geom.point_inside((8, 1), hull) is False
    assert geom.point_inside((-1, -1), hull) is False


def test_simple_square():
    hull = geom.convex_hull([(0, 0), (1, 1), (0, 4), (4, 0), (4, 4), (2, 2)])
    assert hull == [(0, 0), (4, 0), (4, 4), (0, 4)]
    assert geom.point_inside((0, 5), hull) is False
    assert geom.point_inside((1, 1), hull) is True
    assert geom.point_inside((3.9, 3.9), hull) is True


def test_duplicate_points():
    points = [(0, 0), (0, 0), (1, 1), (0, 0)]
    assert len(geom.convex_hull(points)) == 2


@pytest.mark.xfail(reason="Inconsequential edge case, not implemented yet")
def test_convex_hull_points_on_the_edge():
    # to keep polygons minimal the points on an existing edge should
    # ideally not be included in the convex hull
    points = [(0, 0), (2, 0), (0, 2), (2, 2), (0, 1)]
    assert len(geom.convex_hull(points)) == 4
    points = [(0, 0), (2, 0), (0, 2), (2, 2), (2, 1)]
    assert len(geom.convex_hull(points)) == 4


def test_many_points():
    points = [(0.3215348546593775, 0.03629583077160248),
    ...
    ans = [(-0.4907368011686362, 0.1865826865533206),
           (-0.4404289572876217, -0.2894855991839297),
           (-0.161920957418085, -0.4055339716426413),
           (0.4823896228171788, -0.4776170002088109),
           (0.4916198379282093, -0.345391701297268),
